...
export default function Analytics() {
  ...
  const stats = [
    ...
    { label: 'Ümumi Qazanc', value: state.profile.statsVisible ? `${state.profile.totalEarnings} AZN` : '•••••', icon: TrendingUp, color: 'text-teal-400', bg: 'bg-teal-500/10' },
    { label: 'Dəstəkçilər', value: state.donations.length, icon: Users, color: 'text-amber-400', bg: 'bg-amber-500/10' },
  ];
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button 
          onClick={toggleStats}
          className="flex items-center justify-center p-3 border rounded-2xl transition-all shadow-sm active:scale-95 bg-neutral-900/50 border-neutral-800 hover:bg-neutral-800 text-white backdrop-blur-md"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {stats.map((stat, i) => (
          <motion.div
            ...
            key={stat.label}
            className="p-8 rounded-[2rem] border shadow-sm transition-all duration-500 group hover:shadow-xl hover:-translate-y-1 bg-neutral-900/40 border-neutral-800/50 backdrop-blur-xl"
          >
            ...
            <p className="text-sm font-bold tracking-wide uppercase opacity-60 text-neutral-400">{stat.label}</p>
            <h3 className="text-3xl font-display font-bold mt-2 tracking-tight text-white">{stat.value}</h3>
          </motion.div>
        ))}
      </div>
